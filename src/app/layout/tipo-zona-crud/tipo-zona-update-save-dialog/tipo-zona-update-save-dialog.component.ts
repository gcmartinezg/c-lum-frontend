...
export class TipoZonaUpdateSaveDialogComponent implements OnInit {
  ...
  actualizar() {
    if(this.tipoZona.tipoZonaId==null || this.tipoZona.tipoZonaId<=0){
      return this.snackBar.open('Revise el Id por favor', 'ERROR', {
        duration: 3000,
      });
    }
    if(this.tipoZona.nombreTipoZona==null || this.tipoZona.nombreTipoZona==""){
      return this.snackBar.open('Revise el nombre por favor', 'ERROR', {
        duration: 3000,
      });
    }
    if(this.tipoZona.idEstado_Estado==null ){
